...
class FVehicleInputJoyComponent_Handler : public FObjectDatasetMongDBHandler
{
public:
	FVehicleInputJoyComponent_Handler(UVehicleInputJoyComponent* Outer)
	...
	virtual bool Sync(bsoncxx::builder::stream::document& Doc) override
	{
		...
		Doc
			...
			<< "FeedbackDiffFactor" << Outer->GetFeedbackDiffFactor()
			<< "FeedbackResistionFactor" << Outer->GetFeedbackResistionFactor()
			<< "FeedbackAutocenterFactor" << Outer->GetFeedbackAutocenterFactor()
			<< "FeedbackFullFactor" << Outer->GetFeedbackFullFactor()
			<< "FeedbackDriverSteerTension" << Outer->GetDriverInputSteerTension();
		return true;
	}

	TWeakObjectPtr<UVehicleInputJoyComponent> Outer;
};
