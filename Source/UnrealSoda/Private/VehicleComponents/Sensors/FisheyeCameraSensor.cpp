...
UFisheyeCameraSensorComponent::UFisheyeCameraSensorComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
...
void UFisheyeCameraSensorComponent::UninitializeComponent()
{
...
void UFisheyeCameraSensorComponent::ComputeProjectionMap(TArray<FVector>& ProjMap) const
{
	ProjMap.Empty();
	...
	if (bStoreTempProjMap && FPaths::FileExists(*TmpFileName))
	{
		...
		{
			UE_LOG(LogSoda, Error, TEXT("UFisheyeCameraSensorComponent::ComputeProjectionMap(); Can't load fisheye map '%s'"), *TmpFileName);
		}
		else
		{
			...
			{
				UE_LOG(LogSoda, Error, TEXT("UFisheyeCameraSensorComponent::ComputeProjectionMap(); \"%s\" not suitable; Red %i , expect %i"), *TmpFileName, ProjMap.Num(), ExpectSize);
				ProjMap.Empty();
			}
		}
	}
	...
	if (ProjMap.Num() == 0)
	{
		...
		if (bStoreTempProjMap)
		{
			...
			{
				UE_LOG(LogSoda, Error, TEXT("UFisheyeCameraSensorComponent::ComputeProjectionMap(); Can't save fisheye map to '%s'"), *TmpFileName);
			}
		}
	}
}

bool UFisheyeCameraSensorComponent::ApplyProjectionMap(const TArray<FVector> & ProjMap)
{
	...
	for(int i = 0; i < FisheyeSides.Num(); ++i)
	{
		...
		FTexData * TextureData = (FTexData*)FisheyeSides[i].UVTexture->PlatformData->Mips[0].BulkData.Lock(LOCK_READ_WRITE);
		check(TextureData);
		...
		FisheyeSides[i].UVTexture->PlatformData->Mips[0].BulkData.Unlock();
...
void UFisheyeCameraSensorComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
bool UFisheyeCameraSensorComponent::OnActivateVehicleComponent()
{
...
void UFisheyeCameraSensorComponent::OnDeactivateVehicleComponent()
{
...
void UFisheyeCameraSensorComponent::RecalculateProjection()
{
	if (FPaths::FileExists(*TmpFileName))
	{
		IFileManager::Get().Delete(*TmpFileName);
...
bool UFisheyeCameraSensorComponent::GenerateFOVMesh(TArray<FSensorFOVMesh>& Meshes)
{
...
void UFisheyeCameraSensorComponent::MakeScreenshot(const FString& InFileName)
{
