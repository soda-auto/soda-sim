...
URadarBaseSensorComponent::URadarBaseSensorComponent(const FObjectInitializer& ObjectInitializer)
...
bool URadarBaseSensorComponent::OnActivateVehicleComponent()
{
...
void URadarBaseSensorComponent::OnDeactivateVehicleComponent()
{
	Super::OnDeactivateVehicleComponent();
}

void URadarBaseSensorComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
...
void URadarBaseSensorComponent::ProcessHit(const FHitResult* Hit, const FRadarParams* Params)
{
	check(Params && Hit);
	...
	FVector LocalHitPosition = RadarHit.SensorView.InverseTransformPosition(RadarHit.HitPosition);
	LocalHitPosition.Z = 0;
	RadarHit.HitPosition.Z = RadarHit.SensorView.TransformPosition(LocalHitPosition).Z;
...
void URadarBaseSensorComponent::ShowDebudPoints()
{
...
void URadarBaseSensorComponent::ProcessRadarBeams(const FRadarParams & RadarParams)
{
...
void FRadarClusters::AddHit(const FRadarHit& RadarHit, const URadarBaseSensorComponent& Radar)
{
...
void FRadarObjects::FinishScan(const URadarBaseSensorComponent& Radar)
{
	TTimestamp Timestamp = SodaApp.GetSimulationTimestamp();
...
void FRadarObjects::AddHit(const FRadarHit& RadarHit, const URadarBaseSensorComponent& Radar)
{
...
void FRadarObjects::SortObjectsByDistance(TArray<FRadarObject* >& SortedObjects)
{
...
bool URadarBaseSensorComponent::AddFOVMesh(const FRadarParams& Params, TArray<FSensorFOVMesh>& OutMeshes)
{
...
bool URadarBaseSensorComponent::GenerateFOVMesh(TArray<FSensorFOVMesh>& Meshes)
{
	Meshes.Empty();
	bool bRet = false;
...
bool URadarBaseSensorComponent::NeedRenderSensorFOV() const
{
	const TArray<FRadarParams>& RadarParams = GetRadarParams();
...
FBoxSphereBounds URadarBaseSensorComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	FBox Box;
	const TArray<FRadarParams>& RadarParams = GetRadarParams();
	for (auto& It : RadarParams)
