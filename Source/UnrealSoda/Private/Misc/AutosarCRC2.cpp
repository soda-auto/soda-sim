// Copyright 2023 SODA.AUTO UK LTD. All Rights Reserved.

#include "Soda/Misc/AutosarCRC2.h"

static const uint8_t crc8_9B_table[] = 
{
	
};


static uint8_t crc8_ars430_table(uint8_t init_value, uint8_t data_value) 
{
	uint8_t crc = init_value;
	crc = crc8_9B_table[crc ^ data_value];
	return crc;
}

uint8_t CalculateCRC8_2(const uint8_t* Crc_DataPtr, uint32_t Crc_Length, uint8_t Crc_StartValue8, bool Crc_IsFirstCall) 
{
	uint8_t crc = 0U;
	uint32_t data_size = Crc_Length;
	uint32_t data_pos = 0U;

	if (Crc_DataPtr == NULL) {

	}
	else if (Crc_Length == 0U) {

	}
	else {
		if (Crc_IsFirstCall == false) {
			crc = Crc_StartValue8;
		}
		else {
			crc = 0xFFU;
		}
		/* Loop over data array */
		while (data_size > 0U) {

			crc = crc8_ars430_table(crc, Crc_DataPtr[data_pos]);

			data_pos++;
			data_size--;
		}
	}

	return crc;
}