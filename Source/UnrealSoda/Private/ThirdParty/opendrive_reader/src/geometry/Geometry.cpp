...
const DirectedPoint GeometryArc::PosFromDist(double dist) const
{
  ...
  const double theta = _heading - M_PI/2;
...
{
  double u = dist;
  double v = evaluate_polynomial(_a, _b, _c, _d, u);
  ...
  double tangentV = evaluate_polynomial(_b, 2.0 * _c, 3.0 * _d, 0, u);
...
const DirectedPoint GeometryParamPoly3::PosFromDist(const double dist) const
{
  ...
  double u = evaluate_polynomial(_aU, _bU, _cU, _dU, p);
  double v = evaluate_polynomial(_aV, _bV, _cV, _dV, p);
  ...
  double tangentU = evaluate_polynomial(_bU, 2.0 * _cU, 3.0 * _dU, 0.0, p);
  double tangentV = evaluate_polynomial(_bV, 2.0 * _cV, 3.0 * _dV, 0.0, p);
