...
double laneWidth(std::vector<LaneWidth> const &laneWidthVector, double s)
{
  ...
  for (auto it = laneWidthVector.rbegin(); it != laneWidthVector.rend(); it++)
  {
    if (s >= it->soffset)
    {
      ...
      return evaluate_polynomial(it->a, it->b, it->c, it->d, s - it->soffset);
    }
  }
  return 0.0;
}
...
bool GenerateRoadMarkLines(RoadInformation& roadInfo, std::vector<LaneMark>& LaneMarkers, float accuracy)
{
    bool ok = true;
    auto roadId = roadInfo.attributes.id;
    auto& laneSections = roadInfo.lanes.lane_sections;
    ...
    */
    ...
    for (auto it = laneSections.begin(); it != laneSections.end(); it++)
    {
        ...
        auto EvalBorderOffset = [&](const std::vector<LaneInfo>& LanesInfo, const LaneInfo& target_line, double s)
        {
            double borderOffset = 0.;
            for (auto& laneInfo : LanesInfo)
            {
                ...
                if (&laneInfo == &target_line) break;
            }

            return borderOffset;
        };

        auto AddMarksFormLineInfo = [&](const std::vector<LaneInfo>& LanesInfo, const LaneInfo& laneInfo, int dir)
        {
            double mark_s0 = laneInfo.road_marker[0].soffset;

            for (int i = 0; i < laneInfo.road_marker.size(); ++i)
            {
                double mark_s1 = (i == laneInfo.road_marker.size() - 1) ? (s1 - s0) : laneInfo.road_marker[i + 1].soffset;

                if (mark_s1 < mark_s0)
                {
                    std::cerr << "Invalid lane mark length mark_s1, mark_s0\n";
                    break;
                }

                LaneMark laneMark;
                laneMark.type = laneInfo.road_marker[i].type;

             
                const double mark_len = mark_s1 - mark_s0 - 0.001;

                int count = int(mark_len / accuracy + 1);

                const double step = double(mark_len) / count;

                for (int j = 0; j <= count; ++j)
                {
                    double s = s0 + mark_s0 + step * j;
                    auto centerPoint = centerLine.eval(s);
                ...
                if (laneMark.Edge.size() > 1) LaneMarkers.push_back(laneMark);
        ...
        for (auto& laneInfo : laneSection.left) 
            AddMarksFormLineInfo(laneSection.left, laneInfo, 1);
        for (auto& laneInfo : laneSection.right) 
            AddMarksFormLineInfo(laneSection.right, laneInfo, -1);
        AddMarksFormLineInfo(laneSection.center, laneSection.center[0], 0);
