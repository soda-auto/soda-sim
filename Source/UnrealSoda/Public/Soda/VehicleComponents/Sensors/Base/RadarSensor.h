...
#include "RadarSensor.generated.h"
...
struct UNREALSODA_API FRadarCluster
{
	...
	/** Cluster position in local worldspace */
...
struct UNREALSODA_API FRadarClusters
{
	...
	void AddHit(const FRadarHit & Hit, const URadarSensor& Radar);
};
...
struct UNREALSODA_API FRadarObjects
{
	...
	void ResetScan();
	void FinishScan(const URadarSensor& Radar);
	void AddHit(const FRadarHit& Hit, const URadarSensor & Radar);
	uint8 GetFreeRadarObjectId();
	uint8 GetRadarObjectIdForActorId(uint32 ActorId);
	void SortObjectsByDistance(TArray<const FRadarObject* > & SortedObjects) const;
};
...
class UNREALSODA_API URadarSensor : public USensorComponent
{
