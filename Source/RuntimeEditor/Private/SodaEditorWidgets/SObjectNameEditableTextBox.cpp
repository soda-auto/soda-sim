...
int32 SObjectNameEditableTextBox::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	if( TimeSinceHighlightInteraction <= HighlightTargetEffectDuration )
	{
		...
		const FSlateBrush* StyleInfo = FSodaStyle::GetBrush("DetailsView.NameChangeCommitted");
...
FText SObjectNameEditableTextBox::GetNameTooltipText() const
{
	...
	else if (Objects.Num() > 1)
	{
		if (CanEditNameText())
		{
			Result = LOCTEXT("EditableActorLabel_MultIToolActortip", "Rename multiple selected actors at once");
		}
		else
		{
...
{
	auto AllowActorName = [](const UWorld* World, const FText& NewText)
	{
		for (TActorIterator<AActor> It(World); It; ++It)
		{
			if (It->GetName() == NewText.ToString()) return false;
		}
		return true;
	};

	auto AllowComponentName = [](const AActor* Owner, const FText& NewText)
	{
		for (auto& Component : Owner->GetComponents())
		{
			if (Component->GetName() == NewText.ToString()) return false;
		}
		return true;
	};
	...
	FText ErrorMessage;
	if (InTextCommit != ETextCommit::OnCleared && FRuntimeEditorUtils::ValidateActorName(NewText, ErrorMessage))
	{
			...
			if (Objects.Num() == 1)
			{
				if(AActor* Actor = Cast<AActor>(Objects[0].Get()))
				{
					...
					*/
					...
					if (AllowActorName(Actor->GetWorld(), NewText))
					{
						if (Actor->Rename(*TrimmedText.ToString()))
						{
							LastCommittedTime = FSlateApplication::Get().GetCurrentTime();
							RegisterActiveTimer( 0.f, FWidgetActiveTimerDelegate::CreateSP( this, &SObjectNameEditableTextBox::UpdateHighlightSpringState));
						}
				...
				else if (UActorComponent* ActorComponent = Cast<UActorComponent>(Objects[0].Get()))
				{
					if (AllowComponentName(ActorComponent->GetOwner(), NewText))
					{
						if (ActorComponent->Rename(*TrimmedText.ToString()))
						{
							LastCommittedTime = FSlateApplication::Get().GetCurrentTime();
							RegisterActiveTimer( 0.f, FWidgetActiveTimerDelegate::CreateSP( this, &SObjectNameEditableTextBox::UpdateHighlightSpringState));
						}
			...
			else if (Objects.Num() > 1)
			{
				...
				for (int32 i=0; i<Objects.Num(); i++)
				{
					...
					if(Objects[i].IsValid() && Objects[i].Get()->IsA(AActor::StaticClass()))
					{
						...
						*/
						if (AActor* Actor = Cast<AActor>(Objects[i].Get()))
						{
							/*
							if (Actor->IsActorLabelEditable())
							{
						...
						else if (UActorComponent* ActorComponent = Cast<UActorComponent>(Objects[i].Get()))
						{
							if (AllowComponentName(ActorComponent->GetOwner(), NewText))
							{
								if (ActorComponent->Rename(*TrimmedText.ToString()))
...
void SObjectNameEditableTextBox::OnTextChanged( const FText& InLabel )
{
	...
	if(!FRuntimeEditorUtils::ValidateActorName(InLabel, OutErrorMessage))
	{
