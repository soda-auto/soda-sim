...
void AV2XViewer::BeginPlay()
{
	...
	if(ListenSocket)
	{
		...
		{
			UE_LOG(LogSoda, Warning, TEXT("AV2XViewer::BeginPlay(). Can't set SetReuseAddr(true)"));
		}
...
void AV2XViewer::TickActor(float DeltaTime, enum ELevelTick TickType, FActorTickFunction& ThisTickFunction)
{
	...
	for(auto & Obj : V2XObjects)
	{
		...
		if (Obj.Value.TracedValid)
		{
			...
			if (Obj.Value.Data.object_type & (uint32_t)soda::sim::proto_v1::V2XRenderObjectType::OBSTACLE)
			{
				++ObstacleNum;
			...
			else if (Obj.Value.Data.object_type & (uint32_t)soda::sim::proto_v1::V2XRenderObjectType::COLLECTABLE)
			{
				++CollectableNum;
...
{
	if(ArrayReaderPtr->Num() != sizeof(soda::sim::proto_v1::V2XRenderObject))
	{
		UE_LOG(LogSoda, Error, TEXT("AV2XViewer::Recv(); Protocol error, recve %d bytes, was expected %d bytes"), ArrayReaderPtr->Num(), sizeof(soda::sim::proto_v1::V2XRenderObject));
		return;
	}

	soda::sim::proto_v1::V2XRenderObject GenericObject;
	::memcpy(&GenericObject, ArrayReaderPtr->GetData(), sizeof(soda::sim::proto_v1::V2XRenderObject));
